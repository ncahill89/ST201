...
-   Our goal is to obtain coefficient estimates $\hat{\beta}_0$ and $\hat{\beta}_1$ such that the linear model fits the available data closely.

-   There are a number of ways of measuring closeness. However, by far the most common approach involves minimizing minimizing the sum of squared errors.
...
-   Let $\hat{y_i} = \hat{\beta}_0 + \hat{\beta}_1x_i$ be the prediction for $y_i$ based on $x_i$

-   Then $e_i = y_i - \hat{y}_i$ represents the ith residual (error)
...
## Assessing the Accuracy of the Coefficient Estimates {style="font-size: 65%;"}
...
-   We have established that the average of $\hat{\mu}$’s over many sample data sets will be very close to $\mu$ (CLT)

-   But how far off will a single estimate of $\hat{\mu}$ be?

-   In general, we answer this question by computing the standard error of $\hat{\mu}$ with the formula
...
$$\hat{\beta}_0 \pm T_{1-\alpha/2, df}*SE(\hat{\beta}_0)$$

## Significance Testing for the Coefficient Estimates {style="font-size: 65%;"}
...
## The Residual Standard Error (RSE) {style="font-size: 65%;"}
...
## The R$^2$ Statistic {style="font-size: 65%;"}
...
## Checking Model Assumptions {style="font-size: 65%;"}
...
-   This implies a few things
    ...
    -   the difference between $y_i$ and $\hat{y_i}$ (which is e_i) needs to follow a normal distribution
...
Have a look at the data set and see which variable appears to have the strongest predictive relationship with `strain`. Fit a linear regression model using the chosen predictor variable and interpret the output.
