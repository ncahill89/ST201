...
-   Area under the curve is given by a different function called the cumulative distribution function (abbreviated as cdf). The cumulative distribution function, F(x), is used to evaluate probability as area.
...
We will look at 3 'special' types of continuous distributions. These are the:
...
::: {layout-ncol="2"}
![](images/_uniform.png){width="600"}

![](images/_exponential.png){width="600"}
:::

![](images/_normal.png){fig-align="center" width="600"}
...
## The Uniform Distribution {style="font-size: 70%;"}
...
$$E[X] = \int_a^bxf(x) $$ **Definition:** The variance of a discrete random variable is defined as

$$Var[X] = E[X^2] - E[X]^2$$

**E\[X\] and Var\[X\] for a uniform random variable:** Formulas for the theoretical mean and standard deviation are
...
## The Exponential Distribution {style="font-size: 70%;"}

The exponential distribution is often concerned with the amount of time until some specific event occurs.

**Example**

Let $X$ = time in minutes between arrivals at a supermarket checkout. On average there are 2 arrivals per minute. We can model this as $X \sim$ exponential($\lambda=2$).
...
-   The parameters $\mu$ and $\sigma$ are the mean and standard deviation respectively. E\[$X$\] = $\mu$ and Var($X$) = $\sigma^2$.
